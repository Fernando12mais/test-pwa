<script setup lang="ts">
  ...
  const resolveInvoiceStatusVariantAndIcon = (status: string) => {
    ...
    if (status === 'Sent') return { variant: 'primary', icon: 'tabler-mail' };
    if (status === 'Past Due')
...
<template>
  <VCard v-if="invoices" id="invoice-list">
    <VCardText class="d-flex align-center flex-wrap gap-3">
      <!-- 👉 Rows per page -->
      <div class="me-2" style="width: 5rem">
        <VSelect
          v-model="rowPerPage"
          variant="outlined"
          :items="[10, 20, 30, 50]"
            ...
            <VBtn icon variant="text" color="default" size="x-small">
              ...
              <VMenu activator="parent">
                <VList density="compact">
                  <VListItem value="#">
                    <template #prepend>
                      <VIcon size="24" class="me-3" icon="tabler-download" />
                    </template>
                  ...
                  <VListItem value="#">
                    <template #prepend>
                      <VIcon size="24" class="me-3" icon="tabler-stack" />
                    </template>
    ...
    <!-- SECTION Pagination -->
    <VCardText
      class="d-flex align-center flex-wrap justify-space-between gap-4 py-4"
    >
      <!-- 👉  Pagination meta -->
