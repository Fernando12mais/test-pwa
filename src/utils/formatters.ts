...
export function formatMoney(value: number | string) {
  ...
  const formattedValue = intl.format(Number(value));

  const decimals = formattedValue.split(',')[1];

  if (!decimals?.length) {
    return formattedValue.padEnd(formattedValue.length + 3, ',00');
  }

  if (decimals?.length < 2) {
    return formattedValue.padEnd(formattedValue.length + 1, '0');
  }
